# OpenLLM Index — Curated Open Source LLM Decision Engine

Welcome to the **OpenLLM Index**! This is a developer-first, visual directory and VRAM calculator for open-source Large Language Models (LLMs).

This application is built with React, Vite, and Tailwind CSS. It is configured to run fully on-device as a client-side single-page application, making it 100% compatible with GitHub Pages hosting via the `gh-pages` branch.

---

## 🚀 Branch & Deployment Architecture

This project maintains two primary branches to ensure seamless updates and deployments:

### 1. `main` or `master` Branch (Source Code)
* **Purpose**: Houses the React/TypeScript source code, build scripts, styling, and this master `README.md`.
* **Workflow**: Make all code edits, styling additions, and feature development on this branch.
* **Build & Deploy Command**:
  ```bash
  npm run build
  npm run deploy
  ```
  The deploy command uses `gh-pages` to bundle files inside `dist/` and push them directly to the `gh-pages` branch.

### 2. `gh-pages` Branch (Production Deployment)
* **Purpose**: Serves the live production build.
* **Workflow**: Automatically maintained by the `npm run deploy` script.
* **dynamic updates**: Since the build pipeline copies this `README.md` to `dist/README.md`, any update to this README file on the main branch will be bundled and pushed to the live website, which dynamically fetches and parses it at runtime!

---

## ✍️ How to Add or Update Models (For Contributors)

To add a new open-source model or modify an existing one (e.g., benchmark curves, Ollama identifier, context size), you **only need to edit the JSON block** inside this `README.md` file! 

The application will automatically detect, parse, and render your changes in real-time.

### Rules for Editing:
1. Ensure your edits are placed strictly between the `<!-- REGISTRY_JSON_START -->
```json
[
  {
    "id": "deepseek-r1",
...
```
<!-- REGISTRY_JSON_END -->` markers.
2. The data must be valid JSON matching the `ModelMetadata` structure.
3. Test your JSON syntax before pushing.

---

## 🗄️ Model Registry Database
...
OpenLLM Index is maintained by **Bala Venkatesh**.

* **Author Profile**: [Bala Venkatesh Profile Website](https://balavenkatesh3322.github.io/bala_venkatesh_profile/)
* **GitHub**: [@balavenkatesh3322](https://github.com/balavenkatesh3322)
* **Kaggle**: [balavenkatesh3322](https://www.kaggle.com/balavenkatesh3322)
* **Email Contact**: [venkateshpnk22@gmail.com](mailto:venkateshpnk22@gmail.com)

Feel free to open issues or pull requests to update model stats or suggest feature integrations!
